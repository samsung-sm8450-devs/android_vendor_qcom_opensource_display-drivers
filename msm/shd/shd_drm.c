...
#define MSM_MODE_FLAG_SHARED_DISPLAY			(1<<0)
		...
		struct drm_display_mode *mode)
{
	...
	ret = sde_connector_get_mode_info(&sde_conn->base, mode, NULL, &mode_info);
	if (ret)
		return ret;
...
{
	struct shd_display *shd_display = (struct shd_display*)display;
	struct drm_bridge *base_bridge = shd_display->bridge;
	struct shd_bridge *bridge = container_of(base_bridge, struct shd_bridge, base);
	 ...
	 */
	if (!(bridge->private_flags & MSM_MODE_FLAG_SHARED_DISPLAY)) {
		struct msm_mode_info base_mode_info;

		sde_connector_get_mode_info(shd_display->base->connector,
				&shd_display->base->mode, NULL, &base_mode_info);
...
{
	struct shd_bridge *bridge = container_of(drm_bridge,
		struct shd_bridge, base);
	bridge->private_flags |= MSM_MODE_FLAG_SHARED_DISPLAY;
	return true;
}
